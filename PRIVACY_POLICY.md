...
Purpose
storage
Stores monitoring preferences and local runtime status in the browser.
alarms
Runs recurring checks at the interval selected by the user.
notifications
Displays a system notification when a possible slot is detected.
tabs
Finds and reuses the permitted portal tab, activates it for an alert, and opens the portal only when the user explicitly chooses Open Portal.
https://www.usvisascheduling.com/*
Reads and interacts with the official portal pages required for the disclosed monitoring workflow.
